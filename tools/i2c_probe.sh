...
for i in `seq 0 127`; do
	printf "Address: 0x%x\n" $i
	csr i2c_txr      $[($i<<1)+0] # address + write
	csr i2c_command  0x90 # START and send address command
	csr i2c_status        # print status register (tells us if response was ACK or NACK)
	csr i2c_txr      0x00 # load one byte of zeros into send register
	csr i2c_command  0x50 # send byte and STOP
done
